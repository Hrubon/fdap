/*
 * A compressed trie data structure implementation aiming at high performance
...
/*
 * Return a pointer to the character array of the node `n`. The character array
 * is allocated just after the array `child` of child pointers.
 */
...
/*
 * Allocate a new node, making sure that it's initial size will be
...
{
	return trie_find(t, key) != NULL;
}

static void trie_print_node(struct trie *t, struct tnode *n, size_t level)
{
	char *a = char_array(n);
	for (size_t i = 0; i < n->nchild; i++) {
